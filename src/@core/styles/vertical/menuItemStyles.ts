...
const menuItemStyles = (verticalNavOptions: VerticalNavState, theme: Theme): MenuItemStyles => {
  ...
  return {
    root: ({ level }) => ({
      ...
      [`&.${menuClasses.subMenuRoot}.fake-submenu .${menuClasses.subMenuExpandIcon}`]: {
        display: 'none'
      },
      [`&.${menuClasses.subMenuRoot}.${menuClasses.open}.fake-submenu > .${menuClasses.button}, &.${menuClasses.subMenuRoot}.fake-submenu > .${menuClasses.button}.${menuClasses.active}`]:
        {
